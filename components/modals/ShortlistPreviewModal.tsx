...
import { X, ExternalLink, Copy, Users, Eye, Calendar, ChevronDown, Info, FileText } from 'lucide-react';
...
interface ShortlistPreviewModalProps {
  isOpen: boolean;
  onClose: () => void;
  shortlist: {
    id: string;
...
export function ShortlistPreviewModal({
  isOpen,
  onClose,
  shortlist,
  onCandidateView,
  onCandidateAction
}: ShortlistPreviewModalProps) {
  ...
  return (
    <>
      {/* Modal Backdrop */}
      <div
        className="modal-backdrop fixed inset-0 z-50 bg-black/40 backdrop-blur-sm transition-opacity"
        onClick={handleBackdropClick}
      >
        {/* Modal Content */}
        <div className="modal-content flex items-center justify-center min-h-screen p-4">
          <div 
            className="relative bg-white rounded-[20px] shadow-[0_24px_48px_rgba(0,0,0,0.12)] w-full max-w-[1200px] max-h-[90vh] flex flex-col"
          ...
          >
            ...
            <div className="candidate-grid flex-1 overflow-y-auto px-12 pb-6">
              <div 
...
// Export backdrop component separately for reusability
export function ModalBackdrop({ 
  ...
  return (
    <div
      className={`modal-backdrop fixed inset-0 z-50 bg-black/40 backdrop-blur-sm transition-opacity ${className}`}
      onClick={handleBackdropClick}
    >
      {children}
    </div>
